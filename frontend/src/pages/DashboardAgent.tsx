...
const DashboardAgent = () => {
  ...
  const [editingContravention, setEditingContravention] =
  ...
  const [ninaQuery, setNinaQuery] = useState("");
  const [ninaResults, setNinaResults] = useState<NinaResult[]>([]);
  const [selectedNina, setSelectedNina] = useState<NinaResult | null>(null);
  ...
  const detectGPS = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (_err) => {
        alert(
          "Impossible de récupérer votre position. Veuillez saisir manuellement.",
        );
        setGpsLoading(false);
      },
      { timeout: 10000 },
  ...
  useEffect(() => {
    const loadMap = async () => {
      try {
        const res = await api.get("/api/geoloc/");
        setMapLocations(res.data.locations || []);
      } catch (error) {
  ...
  useEffect(() => {
    const timer = window.setTimeout(async () => {
      if (ninaQuery.trim().length < 2) {
        setNinaResults([]);
        return;
      }
      try {
        const res = await api.get(
          `/api/recherche-nina/?q=${encodeURIComponent(ninaQuery)}`,
        );
        setNinaResults(res.data.results || []);
      } catch (error) {
        console.error("Erreur recherche NINA", error);
      }
    }, 250);
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: number) => {
      await api.delete(`/api/contraventions/${id}/`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["agentContraventions"] });
    },
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingContravention) {
      updateMutation.mutate({
        id: editingContravention.id,
        data: {
    ...
    if (selectedNina) {
      formData.append("nina_id", String(selectedNina.id));
    }
    if (photo) {
  ...
  const startEdit = (ctr: Contravention) => {
    setEditingContravention(ctr);
    setSelectedInfraction(String(ctr.infraction_details?.id || ""));
    setImmatriculation(ctr.immatriculation_vehicule || "");
    setTypeVehicule(ctr.type_vehicule || "VOITURE");
    setLieu(ctr.lieu_adresse || "");
    setCommune(ctr.commune || "Commune III");
    setNotes(ctr.notes || "");
    setPhoto(null);
    setGpsCoords(null);
    setSelectedNina(null);
  ...
  const handleDelete = (id: number) => {
    if (window.confirm("Voulez-vous vraiment supprimer ce PV ?")) {
  ...
  const enAttente = contraventions.filter(
  ...
  const filteredContraventions = [...contraventions]
    .filter((c) => {
      const haystack =
        `${c.numero} ${c.immatriculation_vehicule} ${c.infraction_details?.libelle || ""} ${c.lieu_adresse || ""}`.toLowerCase();
  ...
  const recentContraventions = filteredContraventions.slice(0, 10);
  ...
  return (
    <div className="flex h-screen bg-gray-50 font-sans overflow-hidden">
      ...
      <div
        className={`fixed inset-y-0 left-0 z-40 w-64 bg-blue-900 text-white flex-col transition-transform transform ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"} md:relative md:translate-x-0 md:flex`}
      >
        <div className="p-6 border-b border-blue-800 flex justify-between items-center">
          <h2 className="text-xl font-bold text-white flex items-center">
            <span className="w-8 h-8 rounded bg-blue-500 flex items-center justify-center mr-3 text-white font-bold">
              P
            </span>
            Agent Panel
          </h2>
        ...
        <div className="flex-1 overflow-y-auto py-4">
          <nav className="space-y-1 px-3">
            <a
              href="#"
              className="flex items-center px-3 py-2.5 bg-blue-800 text-white rounded-lg"
            >
            ...
            <button
              ...
              className="w-full flex items-center px-3 py-2.5 text-blue-200 hover:bg-blue-800 hover:text-white rounded-lg transition-colors"
            >
        ...
        <div className="p-4 border-t border-blue-800">
          <button
            ...
            className="w-full flex items-center justify-center px-4 py-2 border border-blue-700 rounded-lg text-sm font-medium text-blue-200 hover:bg-blue-800 transition-colors"
          >
      ...
      <div className="flex-1 flex flex-col overflow-hidden w-full">
        <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-4 sm:px-6 z-10">
          ...
          <div className="flex items-center space-x-2 sm:space-x-4">
            <button
              onClick={() => setIsModalOpen(true)}
              className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg text-xs sm:text-sm font-medium flex items-center transition-colors"
            >
          ...
          <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div className="relative w-full md:w-80">
              ...
              <input
                ...
                placeholder="Rechercher un PV, plaque ou infraction"
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-gray-100 text-sm">
                  {recentContraventions.map((ctr) => (
                    ...
                    >
                      <td
                        className="px-6 py-4 font-mono font-medium text-gray-900 cursor-pointer"
                      ...
                      <td
                        ...
                        onClick={() => navigate(`/contraventions/${ctr.id}`)}
                      >
                        {new Date(ctr.date_contravention).toLocaleDateString(
                          "fr-FR",
                        )}
                      </td>
                      <td
                        className="px-6 py-4 text-gray-600 max-w-xs truncate cursor-pointer"
                        onClick={() => navigate(`/contraventions/${ctr.id}`)}
                      >
                        {ctr.infraction_details?.libelle || "Inconnue"}
                      </td>
                      <td
                        className="px-6 py-4 font-bold text-gray-900 cursor-pointer"
                        onClick={() => navigate(`/contraventions/${ctr.id}`)}
                      >
                        {parseFloat(ctr.montant.toString()).toLocaleString(
                          "fr-FR",
                        )}{" "}
                        FCFA
                      </td>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
              <h3 className="text-lg font-bold">
                {editingContravention
                  ? "Modifier le PV"
                  : "Dresser un nouveau PV"}
              </h3>
              <button
            ...
            >
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Recherche citoyen via NINA
                </label>
                <input
                  type="text"
                  value={ninaQuery}
                  onChange={(e) => setNinaQuery(e.target.value)}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder="Nom, NINA, téléphone..."
                />
                {ninaResults.length > 0 && (
                  <ul className="mt-2 border rounded-lg divide-y bg-white max-h-36 overflow-auto">
                    {ninaResults.map((item) => (
                      <li
                        key={item.id}
                        className="p-2 text-sm cursor-pointer hover:bg-gray-50"
                        onClick={() => {
                          setSelectedNina(item);
                          setNinaQuery(
                            `${item.nom_complet} (${item.nin_nina})`,
                          );
                          setImmatriculation(
                            item.immatriculation_vehicule || immatriculation,
                          );
                          setLieu(item.adresse_quartier || lieu);
                          setCommune(item.commune || commune);
                        }}
                      >
                        <div className="font-medium">{item.nom_complet}</div>
                        <div className="text-xs text-gray-500">
                          {item.nin_nina} • {item.telephone}
                        </div>
                      </li>
                    ))}
                  </ul>
                ...
                {selectedNina && (
                  <div className="mt-2 rounded-lg bg-green-50 border border-green-200 p-2 text-sm text-green-800">
                    Sélectionné : {selectedNina.nom_complet} •{" "}
                    {selectedNina.nin_nina}
                  </div>
                )}
              </div>
              <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  >
                    ...
                    <option value="MINIBUS_SOTRAMA">
                      Minibus (Sotrama / Dourouni)
                    </option>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder="Ajoutez les circonstances, les témoins, etc."
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                ...
                <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg">
                  <div className="space-y-1 text-center">
                    ...
                    <p className="text-xs text-gray-500">
                      {photo ? photo.name : "PNG, JPG, GIF jusqu'à 10MB"}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-lg border border-blue-100 bg-blue-50 p-3 text-sm text-blue-800">
                <div className="flex items-center gap-2 font-medium">
                  <MapPinned className="w-4 h-4" /> Carte de vos PV géolocalisés
                </div>
                <div className="mt-2 h-48 rounded-lg overflow-hidden">
                  {mapLocations.length > 0 ? (
                      ...
                      {mapLocations.map((loc, i) => (
                        <Marker key={i} position={[loc.lat, loc.lng]}>
                          <Popup>
                            ...
                            {loc.infraction}
                            <br />
                            {parseFloat(loc.montant.toString()).toLocaleString(
                              "fr-FR",
                            )}{" "}
                            FCFA
                          </Popup>
                    ...
                    <div className="flex h-full items-center justify-center text-sm text-gray-500">
                      Aucune géolocalisation pour l’instant.
                    </div>
                  )}
                </div>
              </div>
